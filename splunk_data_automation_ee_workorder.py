...
script_name = 'splunk_data_automation_ee_workorder' 
...
ex_sql_obj = sql_connection.SqlConnector(ex_db_config['host'], ex_db_config['user'], ex_db_config['password'], database='ExceptionEngine')

### Create paths if they do not exist
helper.check_and_create_directory(log_path, add_archive=True)
...
data_map = query_map.splunk_data_automation_ee_workorder_map

### Set email information and file extension
email_to = ['fsautomation@cbre.com']
email_subject = 'Script Notification - splunk_data_automation_ee_workorder.py'
...
date = timestamp_datetime - timedelta(days=1)
...
# Functions
#############################################################
def clean_list_data(data: list):
	# Clean data for splunk
	for i in range(0, len(data)):
		for k in range(0, len(data[i])):
			data[i][k] = str(data[i][k]).replace('|', '') 
			data[i][k] = str(data[i][k]).replace('\r', '')
			data[i][k] = str(data[i][k]).replace('\n', '')
			data[i][k] = str(data[i][k]).replace("\\", "")

#############################################################
# Body with all of the function calls
#############################################################
#############################################################
# Part 1: Pull Data
#############################################################
...
try:
    ...
    for table_name in data_map:
        ...
        query = """SELECT * FROM [[table]] AS WO WHERE CAST(WO.push_datetime AS DATE) = '[[date]]'"""
        query = query.replace('[[table]]', table)
        query = query.replace('[[date]]', date)

        ee_workorder_dataset = []
        # Run query, store information in ee_workorder_dataset
        try:
            logger.log_info(f'Executing query for {table}')

            # Pull data from ExceptionEngine database
            ee_workorder_dataset = ex_sql_obj.get_data(query)

            # Verify the returned dataset is not empty
            assert(ee_workorder_dataset), 'Query returned empty dataset'
            assert(len(ee_workorder_dataset) > 1), 'Query returned empty dataset'
        except Exception as e:
...
#############################################################    
        ...
        if (ee_workorder_dataset):
            helper.clean_lst_for_bulk_insert(ee_workorder_dataset, fieldterminator=field_term)
        else:
...
#############################################################
    # Writed queried dataset to a .psv file
        try:
            ...
            helper.write_to_csv(output_file, ee_workorder_dataset, _delimiter=field_term, _quoteall=True)
